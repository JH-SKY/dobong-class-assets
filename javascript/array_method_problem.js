...
const deliciousFruits = fruits.map((fruit) => `맛있는 ${fruit}`);
...
const squareNumbers = numbers2.map((num) => num ** 2);
...
// 9번. products 배열에서 '전자제품' 카테고리만 필터링한 후,
// 그 결과물에서 다시 "상품명: 가격원" 형태의 문자열 배열을 만들어 출력한다.
const products = [
...
const electonicProducts = products.filter(
  (product) => product.category === "전자제품",
);

console.log(electonicProducts);

const logProducts = electonicProducts.map(
  (product) => `${product.name}명: ${product.price}원`,
);

console.log(logProducts);

const logProducts2 = products
  .filter((product) => product.category === "전자제품")
  .map((product) => `${product.name}명: ${product.price}원`);

console.log(logProducts2);

console.log("-----------------");

// 10번. students 배열에서 점수가 70점 이상인 학생을 뽑아
// "이름(등급)" 형태의 문자열 배열로 변환하여 출력한다.
const students = [
...
// 12번. 게시글 목록(posts)에서 '공지'인 글만 필터링한 뒤,
// 제목(title)만 추출하여
// 가나다순으로 정렬한 배열을 출력한다.
const posts = [
...
const sortedNoticePosts = posts
  .filter((post) => post.isNotice)
  .map((post) => post.title)
  .sort();

console.log(sortedNoticePosts);

console.log("-----------------");

// 13번. 변수 words 배열에서 글자 수가 5글자 이상인 단어들만 추출하여
// 대문자로 변환한 새로운 배열을 만들고 출력한다.
// (대문자 변환의 경우 검색을 통해 찾아보세요.)
const words = ["apple", "banana", "cherry", "date", "elderberry"];
console.log("13번 문제");

const longUpperWords = words
  .filter((word) => word.length >= 5)
  .map((word) => word.toUpperCase());

console.log(longUpperWords);

console.log("-----------------");

// 14번. 변수 orders 배열에서 status가 'completed'인 주문들의
// total 금액을 모두 더한 총합을 계산하여 출력한다.
const orders = [
...
// 15번. 변수 inventory 배열에서 재고(stock)가 0인 상품을 제외한 뒤,
// 가격(price)이 낮은 순서대로 정렬된
//  상품명(name) 배열을 만들어 출력한다.
const inventory = [
...
const movie = movies.find(
  (movie) => movie.rating >= 9.0 && movie.genre === "Drama",
);
...
const newMembers = members.map((member) => {
  if (member.point >= 100) {
    return `${member.name} (정회원)`;
  } else {
    return `${member.name} (준회원)`;
  }
});

const newMembers2 = members.map((member) =>
  member.point >= 100 ? `${member.name} (정회원)` : `${member.name} (준회원)`,
);

console.log(newMembers);
console.log(newMembers2);
